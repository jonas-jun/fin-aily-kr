...
_ACCOUNT_PATTERNS: list[tuple[str, str | list[str], list[str], list[str]]] = [
    ...
    # excludes: 원가·채권·총이익·총손실로 매출원가·매출채권·매출총이익 오매칭 차단
    # "기타"로 기타영업수익 오매칭 차단 (에이피알 사례)
    # "매출" 단독은 대한항공처럼 account_nm이 "매출"인 기업 대응 (후순위 fallback)
    ("revenue",          ["IS", "CIS"], ["매출액", "수익(매출액)", "매출수익", "영업수익", "매출"],  ["원가", "채권", "총이익", "총손실", "기타"]),
